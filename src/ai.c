...
/* Data structure used by the ai_is_valid_target visitor function below */
struct ai_select_target_data {
...
static int ai_too_close_to_target(class Character *character, class Character *target)
{
	int distance;
...
static int ai_try_move_toward(class Character *character, int dx, int dy)
{
...
static int ai_move_away_from_target(class Character *character, class Character *target)
{
	...
	if (dx == 0 || dy == 0) {
		if (ai_try_move_toward(character, dx, dy))
			return 1;
	}

	/* try another vector */
	if (dx != 0) {
		if (ai_try_move_toward(character, 0, 1))
			return 1;
		if (ai_try_move_toward(character, 0, -1))
			return 1;
	}
	...
	if (dy != 0) {
		if (ai_try_move_toward(character, 1, 0))
			return 1;
		if (ai_try_move_toward(character, -1, 0))
			return 1;
	}
...
static bool ai_attack_target(class Character *character, class Character *target)
{
	int distance;
	bool attacked = false;
...
static int ai_is_valid_target(class Character *attacker, class Object *obj)
{
	/* Skip NULL */
	if (! obj)
		return 0;
...
static void ai_select_target_visitor(struct node *node, void *parm)
{
	class Object *obj;
	struct ai_select_target_data *data;
	int distance;

	/* Extract the typed variables from the generic parms */
	obj = (class Object*)node->ptr;
	data = (struct ai_select_target_data *)parm;

	/* Check if this object makes a valid target */
	if (! ai_is_valid_target(data->attacker, obj))
		return;
...
static class Character * ai_select_target(class Character *character)
{
	struct ai_select_target_data data;
	...
	/* Try the old one */
	if (ai_is_valid_target(character,
...
static void ai_idle(class Character *character)
{
	...
	target = ai_select_target(character);
	if (!target) {
		ai_wander(character);
		return;
	}
	...
	if (!character->canSee(target))	{
		if (! character->pathfindTo(target->getPlace(), target->getX(), target->getY())) {
	...
	if (ai_too_close_to_target(character, target)) {
		if (! ai_move_away_from_target(character, target)) {
			/*if (! ai_switch_to_melee_weapon(character))*/
			ai_wander(character);
		}
		return;
	}
	...
	if (!ai_attack_target(character, target)) {
		if ((! character->pathfindTo(target->getPlace(), target->getX(), target->getY()))
		    && character->isOnMap()) {
			ai_wander(character);
		}
...
void ai_party(class Party *party)
{
	 ...
	 * destination then... */
	if (party->getX() == target_x && party->getY() == target_y) {
	...
	d = place_walking_distance(party->getPlace(), party->getX(), party->getY(), target_x, target_y);
