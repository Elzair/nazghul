...
(define (cast-fireball-proc kchar ktarg)
	...
	(define (do-fireball-hit kplace x y power)
		(define (fire-damage kobj)
			(if (kern-obj-is-char? kobj)
				(begin
					(kern-log-msg "Burning!")
					(if (not (has-fire-immunity? kobj))
						(kern-obj-inflict-damage kobj "burning" (kern-dice-roll "2d3+2") kchar)
						(kern-obj-inflict-damage kobj "impact" (kern-dice-roll "1d3") kchar)
				))
				(kern-obj-apply-damage kobj "impact" (kern-dice-roll "1d3"))
				)
		)
		(let ((kloc (mk-loc kplace x y)))
			(if (kern-is-valid-location? kloc)
	...
	(let ((apower (occ-ability-blackmagic kchar)))
		(define (do-fireball-effect kplace x y)
			(do-fireball-hit kplace x y apower)
			(if (> apower 10) (begin
				(do-fireball-hit kplace (+ x 1) y (- apower 5))
				(do-fireball-hit kplace (- x 1) y (- apower 5))
				(do-fireball-hit kplace x (+ y 1) (- apower 5))
				(do-fireball-hit kplace x (- y 1) (- apower 5))
			(if (> apower 15) (begin
				(do-fireball-hit kplace (+ x 1) (+ y 1) (- apower 10))
				(do-fireball-hit kplace (- x 1) (+ y 1) (- apower 10))
				(do-fireball-hit kplace (+ x 1) (- y 1) (- apower 10))
				(do-fireball-hit kplace (- x 1) (- y 1) (- apower 10))
		...
		(cast-missile-proc kchar ktarg t_mfireball))
	)
